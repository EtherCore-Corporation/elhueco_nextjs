...
export default function Gallery({ layout = 'grid', className = '' }) {
  ...
  if (layout === 'masonry') {
    return (
      <div className={`columns-1 sm:columns-2 lg:columns-3 gap-3 sm:gap-4 space-y-3 sm:space-y-4 ${className}`}>
  ...
  return (
    <div className={`grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-6 ${className}`}>
      {images.map((image, index) => (
